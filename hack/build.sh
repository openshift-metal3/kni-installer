...
PACKAGE_PATH="$(go list -e -f '{{.Dir}}' github.com/openshift-metal3/kni-installer)"
...
MODE="${MODE:-release}"
LDFLAGS="${LDFLAGS} -X github.com/openshift-metal3/kni-installer/pkg/version.Raw=$(git describe --always --abbrev=40 --dirty) -X github.com/openshift-metal3/kni-installer/pkg/version.Commit=$(git rev-parse --verify 'HEAD^{commit}')"
TAGS="${TAGS:-}"
...
release)
	...
	then
		LDFLAGS="${LDFLAGS} -X github.com/openshift-metal3/kni-installer/pkg/asset/ignition/bootstrap.defaultReleaseImageOriginal=${RELEASE_IMAGE}"
	fi
